...
สำหรับเครื่องที่ใช้ Docker daemon ให้ตั้งผ่าน `/etc/docker/daemon.json` ตามตัวอย่างนี้ แต่สำหรับ Kubernetes node ในบท kubeadm จะใช้ `containerd` เป็น runtime จึงต้องตั้งค่า registry ฝั่ง containerd เพิ่มในบท Kubernetes ด้วย การตั้ง Docker daemon อย่างเดียวไม่ทำให้ kubelet/containerd pull image จาก HTTP registry ได้
...
## หมายเหตุสำหรับ Kubernetes ที่ใช้ containerd

Kubernetes ใน Lab นี้ใช้ `containerd` ไม่ใช่ Docker เป็น container runtime ดังนั้น worker node ต้องรู้จัก registry `devops-control:5000` ในฝั่ง containerd ด้วย ตัวอย่างแนวทางคือสร้าง config สำหรับ registry host:

```bash
sudo mkdir -p /etc/containerd/certs.d/devops-control:5000
sudo nano /etc/containerd/certs.d/devops-control:5000/hosts.toml
```

ตัวอย่าง `hosts.toml` สำหรับ lab ที่ใช้ HTTP:
...
จากนั้นให้แน่ใจว่า containerd config เปิดใช้ directory นี้ในส่วน registry config path แล้ว restart containerd:
...
บาง version ของ containerd ต้องตั้ง `config_path = "/etc/containerd/certs.d"` ใน `/etc/containerd/config.toml` ก่อน ถ้า worker node ยัง `ImagePullBackOff` ให้ตรวจ log ของ containerd และ kubelet:
...
ถ้า registry ใช้ HTTP ต้องตั้ง insecure registry ให้ถูกกับ runtime ที่ใช้: Docker ใช้ daemon.json, Kubernetes worker ที่ใช้ containerd ต้องตั้งค่าฝั่ง containerd
```
...
- บทก่อนหน้า: [9. Lab 06: Docker Compose สำหรับ Fullstack App](./09-lab-06-docker-compose-fullstack-app.md)
- สารบัญ: [DevOps Lab Lessons](./README.md)
- บทเรียนถัดไป: [11. Lab 08: Git Flow และ Release Version](./11-lab-08-git-flow-release-version.md)
